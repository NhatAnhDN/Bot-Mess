...
const langData = {
    ...
    "vi_VN": {
        ...
        "add.success": "Đã thêm vào danh sách quản trị viên: {added}",
        "remove.success": "Đã xóa khỏi danh sách quản trị viên: {removed}",
        "list": "╭────╮\n quản trị viên \n╰────╯\n{moderators}",
        "error": "Lỗi: {error}"
    },
    "ar_SY": {
            ...
            case "add":
                {
                    ...
                    if (type == "message_reply") {
                        ...
                        if (global.config.ABSOLUTES.some(id => id == userID)) return reply(getLang("alreadyModerator"));
                        global.config.ABSOLUTES.push(String(userID));
                        success.push({
                            id: userID,
                        ...
                        for (const userID in mentions) {
                            if (global.config.ABSOLUTES.some(id => id == userID)) continue;
                            global.config.ABSOLUTES.push(String(userID));
                            success.push({
                                id: userID,
            ...
            case "del":
                {
                    ...
                    if (type == "message_reply") {
                        let userID = messageReply.senderID;
                        if (!global.config.ABSOLUTES.some(id => id == userID)) return reply(getLang("notModerator"));
                        global.config.ABSOLUTES = global.config.ABSOLUTES.filter(id => id != userID);
                        success.push({
                            id: userID,
                            name: (await global.controllers.Users.getInfo(userID))?.name || userID
                        });
