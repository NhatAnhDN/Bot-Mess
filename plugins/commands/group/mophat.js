...
const config = {
   name: "mophat",
   description: " banner with a user",
   usage: "<@mention/reply>",
   credits: "XaviaTeam",
   cooldown: 5
}
...
async function makeImage(data) {
   ...
   try {
      const template = await Jimp.read('https://i.imgur.com/Rw17u2R.jpg');
 ...
 const avatarOne = await Jimp.read(avatarPathOne);
  ...
  template.composite(avatarTwo, 460, 300);
...
const langData = {
   ...
   "vi_VN": {
      ...
      "loveMessage": "mô phật, để ta siêu độ cho túc chủ"
   }
}
